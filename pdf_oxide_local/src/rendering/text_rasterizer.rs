    ...
    let result = (|| {
        ...
        for sub in cmap.subtables {
            ...
            match (sub.platform_id, sub.encoding_id) {
                (PlatformId::Unicode, _) => saw_unicode = true,
                // Windows Unicode BMP (1) or UCS-4 (10)
                (PlatformId::Windows, 1) | (PlatformId::Windows, 10) => saw_unicode = true,
                // Windows Symbol (3): byte-indexed, glyphs stored at 0xF000+byte
                (PlatformId::Windows, 3) => saw_byte_indexed = true,
                // Macintosh Roman (0): byte-indexed
                (PlatformId::Macintosh, 0) => saw_byte_indexed = true,
                _ => {},
            }
        }
        ...
        for sub in cmap.subtables {
            ...
            match (sub.platform_id, sub.encoding_id) {
                ...
                (PlatformId::Windows, 3) => {
                    // Windows Symbol encoding: character codes are stored at 0xF000 + byte.
                    if let Some(gid) = sub.glyph_index(0xF000 + byte as u32) {
                        return Some(gid.0);
                    }
                },
                _ => {},
...
/// Return all installed font family names, sorted alphabetically.
/// Queries the process-wide system fontdb (cheap after first call).
...
/// User-selected CJK fallback font name (empty = automatic).
fn preferred_cjk_font_lock() -> &'static std::sync::Mutex<String> {
    static CELL: std::sync::OnceLock<std::sync::Mutex<String>> = std::sync::OnceLock::new();
    CELL.get_or_init(|| std::sync::Mutex::new(String::new()))
}

/// Set the preferred CJK fallback font by name (empty string = automatic priority list).
/// Invalidates the cache so the next render picks up the new selection.
pub fn set_preferred_cjk_font(name: &str) {
    *preferred_cjk_font_lock().lock().unwrap() = name.to_string();
    if let Some(cache) = CJK_FALLBACK.get() {
        *cache.lock().unwrap() = None;
    }
}

/// Process-wide CJK fallback font cache.
/// `None` = not yet computed; `Some((key, result))` = computed for `key` (preferred font name).
static CJK_FALLBACK: std::sync::OnceLock<
    std::sync::Mutex<Option<(String, Option<(fontdb::ID, Arc<Vec<u8>>, u32)>)>>,
> = std::sync::OnceLock::new();

fn get_cjk_fallback_cached(db: &fontdb::Database) -> Option<(fontdb::ID, Arc<Vec<u8>>, u32)> {
    let preferred = preferred_cjk_font_lock().lock().unwrap().clone();

    let cache_mutex = CJK_FALLBACK.get_or_init(|| std::sync::Mutex::new(None));
    ...
    let result = compute_cjk_fallback(&preferred, db);
    *cache = Some((
        preferred,
    ...
    if !preferred.is_empty() {
        ...
        if let Some(id) = db.query(&query) {
            if let Some((arc, idx)) = cached_font_bytes(id, db) {
                log::debug!("CJK fallback: user preference '{}' found", preferred);
                return Some((id, arc, idx));
            }
        }
    }

    // Automatic priority: JP fonts first (no UI variants), then SC as last resort
    let prioritized_variants = [
        "Yu Gothic",            // Windows 標準 (游ゴシック)
        "Meiryo",               // Windows 標準 (メイリオ)
        "MS Gothic",            // MS ゴシック
        "Yu Mincho",            // 游明朝
        "MS Mincho",            // MS 明朝
        "Noto Sans CJK JP",     // Linux/macOS 日本語
        "Noto Serif CJK JP",
        "Noto Sans CJK SC",
        "Noto Serif CJK SC",
        "Droid Sans Fallback",
...
// ---------------------------------------------------------------------------
// Latin / general fallback font (user-selected)
// ---------------------------------------------------------------------------

fn preferred_latin_font_lock() -> &'static std::sync::Mutex<String> {
    static CELL: std::sync::OnceLock<std::sync::Mutex<String>> = std::sync::OnceLock::new();
    CELL.get_or_init(|| std::sync::Mutex::new(String::new()))
}

/// Set the preferred Latin/general fallback font by name (empty = no fallback).
/// Invalidates the cache so the next render picks up the new selection.
pub fn set_preferred_latin_font(name: &str) {
    *preferred_latin_font_lock().lock().unwrap() = name.to_string();
    if let Some(cache) = LATIN_FALLBACK.get() {
        *cache.lock().unwrap() = None;
    }
}

static LATIN_FALLBACK: std::sync::OnceLock<
    std::sync::Mutex<Option<(String, Option<(fontdb::ID, Arc<Vec<u8>>, u32)>)>>,
> = std::sync::OnceLock::new();
...
impl TextRasterizer {
            ...
            if let Some(ref info) = font_info {
                if let Some(ref embedded) = info.embedded_font_data {
                    ...
                    if has_unicode_cmap {
                        ...
                        // system font on error, so this is safe for all font types including Type1.
                        log::debug!(
                            "Embedded font '{}' cmap unclassified, attempting direct embedded render",
                            info.base_font
                        );
                        Some((None, Arc::clone(embedded), 0, true))
                    }
    ...
    pub fn render_tj_array(
        ...
        let wmode = gs.font_name.as_deref()
            .and_then(|n| font_cache.get(n))
            .map(|fi| fi.wmode)
            .unwrap_or(0);
        ...
        for element in array {
            match element {
                TextElement::String(text) => {
                    ...
                    // Advance text position: vertical (wmode=1) or horizontal (wmode=0)
                    let advance_matrix = if wmode == 1 {
                        crate::content::Matrix::translation(0.0, -advance)
                    } else {
                        crate::content::Matrix::translation(advance, 0.0)
                    };
                ...
                TextElement::Offset(offset) => {
                    ...
                    let advance_matrix = if wmode == 1 {
                        crate::content::Matrix::translation(0.0, -shift)
                    } else {
                        crate::content::Matrix::translation(shift, 0.0)
                    };
                    current_gs.text_matrix = advance_matrix.multiply(&current_gs.text_matrix);
                    total_advance += shift;
                },
    ...
    fn render_unicode_text(
        ...
        // 2. Buffer setup
        let wmode = font_info.map(|fi| fi.wmode).unwrap_or(0);
