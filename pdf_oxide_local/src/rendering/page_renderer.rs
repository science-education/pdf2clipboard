            ...
            match op {
                ...
                Operator::Tj { text } => {
                    if in_text_object {
                        ...
                        let advance_matrix = if wmode == 1 {
                            Matrix::translation(0.0, -advance)
                        } else {
                            Matrix::translation(advance, 0.0)
                        };
                ...
                Operator::Quote { text } => {
                    if in_text_object {
                        ...
                        let wmode_q = gs.font_name.as_deref()
                            .and_then(|n| self.fonts.get(n))
                            .map(|fi| fi.wmode)
                            .unwrap_or(0);
                        let advance = self.text_rasterizer.render_text(
                        ...
                        let gs_mut = gs_stack.current_mut();
                        let advance_matrix = if wmode_q == 1 {
                            Matrix::translation(0.0, -advance)
                        } else {
                            Matrix::translation(advance, 0.0)
                        };
                ...
                Operator::TJ { array } => {
                    if in_text_object {
                        ...
                        let gs = gs_stack.current();
                        let transform = combine_transforms(base_transform, &gs.ctm);
                        let wmode_tj = gs.font_name.as_deref()
                            .and_then(|n| self.fonts.get(n))
                            .map(|fi| fi.wmode)
                            .unwrap_or(0);
                        log::debug!(
                        ...
                        // Advance text position: vertical (wmode=1) or horizontal (wmode=0)
                        let gs_mut = gs_stack.current_mut();
                        let advance_matrix = if wmode_tj == 1 {
                            Matrix::translation(0.0, -advance)
                        } else {
                            Matrix::translation(advance, 0.0)
                        };
                ...
                Operator::DoubleQuote {
                    ...
                    if in_text_object {
                        ...
                        let gs = gs_stack.current();
                        let transform = combine_transforms(base_transform, &gs.ctm);
                        let wmode_dq = gs.font_name.as_deref()
                            .and_then(|n| self.fonts.get(n))
                            .map(|fi| fi.wmode)
                            .unwrap_or(0);
                        log::debug!(
                        ...
                        // Advance text position
                        let gs_mut = gs_stack.current_mut();
                        let advance_matrix = if wmode_dq == 1 {
                            Matrix::translation(0.0, -advance)
                        } else {
                            Matrix::translation(advance, 0.0)
                        };
