...
pub struct FontInfo {
    ...
    /// Writing mode: 0 = horizontal, 1 = vertical (from CMap /WMode or "-V" suffix).
    pub wmode: u8,
}
...
impl FontInfo {
    ...
    pub fn from_dict(dict: &Object, doc: &PdfDocument) -> Result<Self> {
        ...
        // Detect vertical writing mode from the CMap encoding name ("-V" suffix)
        // Per PDF Spec §9.7.6.2: CMap name ending in "-V" indicates WMode=1 (vertical).
        let wmode: u8 = font_dict
            .get("Encoding")
            .and_then(|obj| obj.as_name())
            .map(|n| if n.ends_with("-V") || n.ends_with("Vert") { 1 } else { 0 })
            .unwrap_or(0);
