# PDF Page to Clipboard (Web & Desktop)

PDFのページをサムネイル一覧で表示し、選択したページの画像を高品質でクリップボードにコピーするアプリケーションです。
Windows向けネイティブデスクトップアプリとして動作するほか、WebAssembly (Wasm) を通じてモダンなWebブラウザ上でも「爆速」で動作するクロスプラットフォーム構成となっています。
...
- **高品質クリップボード転送**: Web/デスクトップを問わず、指定した解像度（デフォルト300 DPI）で画像を生成し、そのままシステムのクリップボードへ格納。プレゼン資料やドキュメント作成への貼り付けに最適です。
- **柔軟なファイル読み込み**: PDFファイルのドラッグ＆ドロップはもちろん、**Web上のPDFのURLを直接入力してダウンロード・表示**する機能にも対応（クロスプラットフォーム非同期通信）。
- **堅牢なCORSプロキシチェーン（Web版）**: Webブラウザのセキュリティ制限（CORS）を自動的にバイパスするため、複数の有力プロキシサーバー（`codetabs`, `corsproxy.io`, `allorigins`）を裏で順番に試行する強力なフォールバック機能を搭載。官公庁や論文のPDFでも極めて高いダウンロード成功率を誇ります。
- **進捗の視覚化**: レンダリングやコピーの状況、URLダウンロード中のスピナー表示などをリアルタイムでステータスバーに表示。
...
cargo run --release
```
※ビルドされた実行ファイルは `target/release/PDFPageToClipboard.exe` に出力されます。
